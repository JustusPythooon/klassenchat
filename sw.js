...
self.addEventListener('push',event=>{
 ...
 event.waitUntil((async()=>{
  const badgeCount=Number(data.badgeCount||data.badge||1)||1;
  // If the PWA is open, update its native app badge immediately.
  try{
    const cs=await clients.matchAll({type:'window',includeUncontrolled:true});
    for(const c of cs)c.postMessage({type:'KLASSENCLOUD_BADGE',count:badgeCount});
  }catch{}
  await self.registration.showNotification(data.title,{
    body:data.text,
    icon:'/icon-192.png',
    badge:'/icon-192.png',
    tag:data.tag||'klassencloud-push',
    data:{url:data.url||'/'},
    renotify:true
  });
})())
